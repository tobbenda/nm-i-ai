"""
YOLOv8x trained on ALL 248 images (no val holdout).
Uses best known hyperparams. Train for fixed epoch count (42 — best from split runs).
"""
import json
import shutil
...
def coco_to_yolo():
    ...
    # All images go to train, val gets a tiny subset just to avoid errors
    ...
    # Put all in train, copy last 5 to val too (just for ultralytics validation step)
    ...
    print(f"Dataset: {len(train_imgs)} train, {len(val_imgs)} val, {nc} classes")
...
if __name__ == "__main__":
    ...
    model.train(
        ...
        batch=2,
        device=0,
        patience=50,  # no early stopping — fixed epochs
        save=True,
        project="runs",
        name=NAME,
        optimizer="AdamW",
